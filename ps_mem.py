...
def parse_options():
    ...
    parser.add_argument('-t', '--total', dest='only_total', action='store_true',
    ...
    parser.add_argument('-S', '--swap', dest='show_swap', action='store_true',
        help='Show swap information')
    parser.add_argument('-p', dest='pids', metavar='<pid>[,pid2,...pidN]',
        help='Only show memory usage PIDs in the specified list')
    parser.add_argument('-w', dest='watch', metavar='<N>', type=int,
    ...
    return (args.split_args, args.pids_to_show, args.watch, args.only_total,
            args.discriminate_by_pid, args.show_swap)
...
def getMemStats(pid):
    ...
    if os.path.exists(proc.path(pid, 'smaps')):
        ...
        if os.path.exists(proc.path(pid, 'smaps_rollup')):
            smaps = 'smaps_rollup'
        lines = proc.open(pid, smaps).readlines()
...
def show_val_accuracy(ram_inacc, swap_inacc, only_total, show_swap):
    ...
    if swap_inacc == -1:
        ...
        sys.stderr.write("%s: Swap is over-estimated by this system for each program,\n"
...
def get_memory_usage(pids_to_show, split_args, discriminate_by_pid,
    ...
    for pid in os.listdir(proc.path('')):
        ...
        if shared_huges.get(cmd):
            if shared_huges[cmd] < shared_huge:
...
def print_memory_usage(sorted_cmds, shareds, count, total, swaps, total_swap, show_swap):
    ...
    if have_swap_pss and show_swap:
        sys.stdout.write("%s\n%s%9s%s%9s\n%s\n" %
                         ("-" * 45, " " * 24, human(total), " " * 3,
                          human(total_swap), "=" * 45))
    elif have_pss:
